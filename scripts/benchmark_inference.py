#!/usr/bin/env python3
"""Benchmark inference: latency and memory (PyTorch). Usage: python scripts/benchmark_inference.py --checkpoint checkpoints/best.pt [--sizes 256 512]"""
import argparse
import sys
import time
...
from src.inference.predict import load_model, denoise_image
...
def main():
    ...
    for size in args.sizes:
        ...
        mem = "N/A"
        if device.type == "cuda":
            mem = f"{torch.cuda.max_memory_allocated(device) / 2**20:.1f} MB"
